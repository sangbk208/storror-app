...
export const addToCart = (selectProduct) => {
  ...
  if (!cartProducts) {
    ...
    let product = cartProducts.filter(
      (item) => item.idProduct === selectProduct.idProduct
    );
    if (product.length === 0) {
      localStorage["cart"] = JSON.stringify([
        ...cartProducts,
        { ...selectProduct, id: uuid() },
      ]);
    } else {
      let productEdit = product.filter(
        (item) => item.option === selectProduct.option
      );
      if (productEdit.length === 0) {
        ...
        const newQuantity =
          cartProducts[index].quantity + selectProduct.quantity;
        cartProducts[index].quantity = newQuantity;
        cartProducts[index].totalPrice = newPrice(
...
export const deleteCart = (id) => {
  let cartProducts = JSON.parse(localStorage.getItem("cart"));
  localStorage["cart"] = JSON.stringify(
    Object.values(_.omit({ ..._.mapKeys(cartProducts, "id") }, id))
  );
  return {
    type: DELETE_CART,
    payload: id,
